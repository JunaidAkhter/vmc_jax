...
class MPO(nn.Module):
    ...
    def __call__(self, x):
        ...
        for i, dm in enumerate(inp_dms):
            if i == 0:
                #print('i:', i, 'dm:', dm)
                nodes.append(self.param('a'+str(i),init.cplx_init_node1, (oup_dms[i],dm,D))) # include the node name later
                legs.append([-1,1,n+1])
            elif i == n-1:
                #print('i:', i, 'dm:', dm)
                nodes.append(self.param('a'+str(i),init.cplx_init_node2, (dm,oup_dms[i],D)))
...
def simulate(rng, iterations, D):
    ...
    net = MyNet(num_nodes = 2, inp_dims = (4, 3), oup_dims = (4, 4), D = D) # D = 08 in reality
...
#np.savetxt('cpxmpo_12_avg_d4_43_gs', E_0_aarray, header='Data for rlmpo with D = 4 for 10 different initializations')
