    ...
    def __call__(self, x):
        ...
        for i, dm in enumerate(inp_dms):
            if i == 0:
                ...
                #print('i:', i, 'dm:', dm)
                nodes.append(self.param('a'+str(i), init.cplx_init1, (dm,D,oup_dms[i],D)))
...
def simulate(rng, iterations, time_step): 
...
for j,rng in enumerate(rng_list):
    print("rng:", rng)
    res = simulate(rng, iterations, time_step = time_step)
...
np.savetxt('cpxmpo_12_avg_d8_43_gs_8t', E_0_aarray, header = 'Data for cpxmpo with D = 8 for 1 initializations')
