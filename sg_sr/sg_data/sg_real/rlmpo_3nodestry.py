...
class MPO(nn.Module):
    ...
    def __call__(self, x):
        ...
        for i, dm in enumerate(inp_dms):
            if i == 0:
                #print('i:', i, 'dm:', dm)
                nodes.append(self.param('a'+str(i), partial(init.rl_init1, var = d**2/a), (oup_dms[i],dm,D))) # include the node name later
                legs.append([-1,1,n+1])
            elif i == n-1:
                #print('i:', i, 'dm:', dm)
                nodes.append(self.param('a'+str(i), partial(init.rl_init1, var = d**2/c), (dm,oup_dms[i],D)))
                legs.append([n, -n, 2*n-1])

            else:
                #print('i:', i, 'dm:', dm)
                nodes.append(self.param('a'+str(i),  partial(init.rl_init1, var = d**2/b), (dm,D,oup_dms[i],D)))
...
net = MyNet(num_nodes = 3, inp_dims = (2,3,2), oup_dims = (4,3,3), D=6) 
...
for j,rng in enumerate(rng_list):
    print("rng:", rng)
    res = simulate(rng, iterations, D=6)
...
#np.savetxt('rlmpo_12_avg_d1_232_gs_try', E_0_aarray, header='Data for rlmpo with D = 6 for 10 different initializations')
