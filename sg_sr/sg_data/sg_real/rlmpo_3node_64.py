...
def_net = MyNet(num_nodes = 3, inp_dims = (2,4,8), oup_dims = (18,18,18), D=D) 
params = def_net.init(jax.random.PRNGKey(1),jnp.zeros((L,), dtype=global_defs.tCpx)) # the "dtype" here is not so important
...
for j,rng in enumerate(rng_list):
    ...
    E_0 = res + 1.1153253548387492 #this energy is for 24 spins
...
np.savetxt('rlmpo_64_avg_d9_248_gs', E_0_aarray, header='Data for rlmpo with D = 9 for 10 different initializations')
