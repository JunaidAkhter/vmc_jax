...
def simulate(rng, iterations, h):
    # Initialize net
    net = jVMC.nets.RBM(numHidden = h, bias = False)
...
for j,rng in enumerate(rng_list):
    print("rng:", rng)
    res = simulate(rng, iterations, h=h) # for \alpha = 2
...
np.savetxt('rlrbm_64_avg_a2_gs', E_0_aarray, header='Data for rlrbm with h = 128 for 11 different initializations')
